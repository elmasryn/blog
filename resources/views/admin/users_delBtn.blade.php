<button class="btn btn-danger btn-sm d-flex flex-nowrap" type="button" data-toggle="modal"
    data-target="#delUser{{ $id }}"><i class="fas fa-trash-alt mr-1 my-auto"></i>
    {{ trans('lang.Delete') }}</button>

<!-- Modal -->
<div id="delUser{{ $id }}" tabindex="-1" class="modal fade" role="dialog" aria-labelledby="del" aria-hidden="true">
    <div class="modal-dialog" role="document">
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route(adminview('users.destroy') , $id)}}">
                @csrf
                @method('DELETE')
                <div class="modal-body">
                    <h4>
                        {{ __('lang.Are you sure to delete the user' , ['user' => $name]) }}
                    </h4>
                </div>
