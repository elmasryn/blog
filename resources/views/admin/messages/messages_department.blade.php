...
@section('title' , '| '.$message_title)
@section('mailContent')
@section('breadcrumb')
<li class="breadcrumb-item active">{{ $message_title }}</li>
@endsection
...
<div class="col-md-9">
    <div class="card card-primary card-outline">
        ...
        <div class="card-header">
            ...
            @if(isset($showMessagesInTitle) && count($showMessagesInTitle) > 0)
        ...
        <div class="card-body p-0">
            <div class="mailbox-controls">
                ...
                <div class="float-right">
                    {{$showMessagesInTitle->firstItem()}}-{{$showMessagesInTitle->lastItem()}}/{{$showMessagesInTitle->total()}}
                    <div class="btn-group">
                        <a type="button" href="{{$showMessagesInTitle->nextPageUrl()}}"
                            class="btn btn-default btn-sm {{$showMessagesInTitle->hasMorePages() ? '' : 'disabled'}}"><i
                                class="fas fa-chevron-left"></i></a>
                        <a type="button" href="{{$showMessagesInTitle->previousPageUrl()}}"
                            class="btn btn-default btn-sm {{$showMessagesInTitle->onFirstPage() ? 'disabled' : ''}}"><i
            ...
            <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped">
                    <tbody>
                        <form id="del_form_dep" method="POST" action="">
                            @csrf
                            @method('POST')
                            <input id="my_input" type="hidden" name="">
                            @foreach ($showMessagesInTitle as $message)
